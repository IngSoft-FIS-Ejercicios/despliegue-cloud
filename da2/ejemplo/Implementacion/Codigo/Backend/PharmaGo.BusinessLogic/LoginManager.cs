...
namespace PharmaGo.BusinessLogic
{
    public class LoginManager : ILoginManager
    {
        ...
        public Authorization Login(string userName, string password)
        {
            ...
            var pharmacyId = user.Pharmacy != null ? user.Pharmacy.Id : 0;
            if (session == null) {
                var token = Guid.NewGuid();
                Session newSession = new Session { Token = token, UserId = _userId };
                _sessionRepository.InsertOne(newSession);
                _sessionRepository.Save();
                return new Authorization { Token = token, Role = user.Role.Name, UserName = user.UserName, PharmacyId = pharmacyId };
            }
            return new Authorization { Token = session.Token, Role = user.Role.Name, UserName = user.UserName, PharmacyId = pharmacyId };
        }
