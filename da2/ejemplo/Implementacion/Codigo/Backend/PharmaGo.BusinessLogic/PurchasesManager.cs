    ...
    public class PurchasesManager : IPurchasesManager
    {
        ...
        public Purchase CreatePurchase(Purchase purchase)
        {
            ...
            foreach (var detail in purchase.details)
            {
                ...
                {
                    throw new InvalidResourceException("Quantity must be a positive number greater than 0.");
                }

                string drugCode = detail.Drug.Code;
                var drugs = pharmacy.Products.OfType<Drug>().ToList();
                var drug = drugs.FirstOrDefault(x => x.Code == drugCode && x.Deleted == false);
                if (drug is null)
                {
