...
export class UpdateInvitationComponent implements OnInit {
  ...
  getInvitationById(id: any): void {
    this.invitationService.getInvitationById(id).subscribe((invitation) => {
  ...
  updateInvitation(): void {
    const pharmacyName = this.pharmacy_name.value
      ...
      : '';
    const userName = this.user_name.value ? this.user_name.value : '';
    const roleName = this.role_name.value ? this.role_name.value : '';
    const userCode = this.user_code.value ? this.user_code.value : '';
    const id = this.invitation_id;
