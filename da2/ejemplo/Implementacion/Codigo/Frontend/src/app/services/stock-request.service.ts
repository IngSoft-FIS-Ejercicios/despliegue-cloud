...
export class StockRequestService {
  ...
  getStockRequests(
    ...
    return this.http
      .get<StockRequest[]>(url, { headers: this.getHttpHeaders() })
      .pipe(
        tap(),
        catchError(
          this.handleError<StockRequest[]>(
            'Get Stock Requests By Employee',
            [],
          ),
        ),
      );
  ...
  getStockRequestsByOwner(): Observable<StockRequest[]> {
    return this.http
      .get<StockRequest[]>(this.url, { headers: this.getHttpHeaders() })
      .pipe(
        tap(),
        catchError(
          this.handleError<StockRequest[]>('Get Stock Requests By Owner', []),
        ),
      );
  }
  ...
  createStockRequest(request: RequestDetailHeader[]): Observable<any> {
    const req = new RequestDetail(request);
    return this.http
      .post<any>(this.url, req, { headers: this.getHttpHeaders() })
      .pipe(tap(), catchError(this.handleError<any>('Create Stock Request')));
  }

  approveStockRequest(requestId: number): Observable<any> {
    const url = `${this.url}/approveStockRequest/${requestId}`;
    return this.http
      .put<any>(url, requestId, { headers: this.getHttpHeaders() })
      .pipe(tap(), catchError(this.handleError<any>('Approve Stock Request')));
  }
  ...
  rejectStockRequest(requestId: number): Observable<any> {
    ...
    return this.http
      .put<any>(url, requestId, { headers: this.getHttpHeaders() })
      .pipe(tap(), catchError(this.handleError<any>('Reject Stock Request')));
  }
