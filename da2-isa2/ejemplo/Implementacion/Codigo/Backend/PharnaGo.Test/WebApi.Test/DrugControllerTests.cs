...
namespace PharmaGo.Test.WebApi.Test
{
    ...
    public class DrugControllerTests
    {
        ...
        {
            _drugManagerMock.Setup(x => x.Create(It.IsAny<Drug>())).Returns(drug);
        ...
        {
            _drugManagerMock.Setup(x => x.Create(It.IsAny<Drug>())).Throws(new InvalidResourceException(invalidResourceExceptionMessage));
        ...
        {
            _drugManagerMock.Setup(x => x.Create(It.IsAny<Drug>())).Throws(new ResourceNotFoundException(resourceNotFoundExceptionMessage));
            var result = _drugController.Create(drugModel);
            var objectResult = result as ObjectResult;
            var statusCode = objectResult.StatusCode;
            _drugManagerMock.VerifyAll();
