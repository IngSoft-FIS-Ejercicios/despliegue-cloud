...
namespace PharmaGo.Test.WebApi.Test
{
    ...
    [TestCleanup]
        ...
        [TestMethod]
        public void GetPharmacyByToken_ReturnsPharmacyOk()
        {
            var token = "c80da9ed-1b41-4768-8e34-b728cae25d2f";
            var pharmacyDetailModel = new PharmacyDetailModel(pharmacy);

            _pharmacyManagerMock.Setup(x => x.GetByEmployee(token)).Returns(pharmacy);

            _pharmacyController.ControllerContext.HttpContext = new DefaultHttpContext();
            _pharmacyController.ControllerContext.HttpContext.Request.Headers["Authorization"] = token;

            var result = _pharmacyController.GetByEmployee();

            var objectResult = result as ObjectResult;
            Assert.AreEqual(200, objectResult.StatusCode);

            var returnedPharmacy = objectResult.Value as PharmacyDetailModel;
            Assert.IsNotNull(returnedPharmacy);
            Assert.AreEqual(pharmacyDetailModel.Name, returnedPharmacy.Name);
            Assert.AreEqual(pharmacyDetailModel.Address, returnedPharmacy.Address);
        }
