...
namespace PharmaGo.BusinessLogic
{
    public class DrugManager : IDrugManager
    {
        ...
        public Drug Create(Drug drug)
        {
            ...
            Pharmacy pharmacyOfDrug = _pharmacyRepository.GetOneByExpression(p => p.Name == drug.Pharmacy.Name);
            if (pharmacyOfDrug == null)
            {
                throw new ResourceNotFoundException("The pharmacy of the drug does not exist.");
            }

            if (_drugRepository.Exists(d => d.Code == drug.Code && d.Pharmacy.Id == pharmacyOfDrug.Id))
            {
            ...
            drug.UnitMeasure = unitMeasureOfDrug;
            drug.Presentation = presentationOfDrug;
            drug.Pharmacy = pharmacyOfDrug;
